...
<body class="bg-gray-900 text-white min-h-screen font-body relative">
    ...
    <div class="fixed top-0 left-0 w-full h-full pointer-events-none z-[-1] overflow-hidden">
        <div class="absolute top-[-20%] right-[-10%] w-[800px] h-[800px] rounded-full blur-[180px] opacity-20"
    ...
    <main class="pt-36 pb-16 max-w-4xl mx-auto px-4 md:px-8">
        ...
        <div class="mb-10 w-full h-[400px] rounded-3xl overflow-hidden relative border border-white/10 shadow-2xl">
            ...
            <div class="absolute bottom-0 left-0 w-full p-8">
                <span
                    class="px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full backdrop-blur-md bg-white/10 text-white border border-white/20 mb-4 inline-block"
                    style="color: <?= $colorValue ?>; border-color: <?= $colorValue ?>;">
                    <?= htmlspecialchars($artTypeLabel) ?>
                </span>
                <h1
                    class="text-4xl md:text-6xl font-bold font-display uppercase tracking-wider mb-2 text-white drop-shadow-lg">
        ...
        <article
            class="prose prose-invert max-w-none prose-a:text-[<?= $colorValue ?>] prose-a:no-underline hover:prose-a:underline prose-headings:font-display prose-headings:uppercase prose-headings:tracking-wider">
            <div class="space-y-6">
                <?php echo markdownToHtml($event['markdown']); ?>
            </div>
        </article>

        <!-- Back Footer & Participation Form -->
        <footer class="mt-16 pt-8 border-t border-white/10">
            <div class="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
                ...
                <div class="participation-container p-4 rounded-2xl border border-white/10 w-full md:w-auto">
                    <form id="participateForm" class="flex flex-col sm:flex-row gap-3 items-center">
                        <input type="hidden" name="eventId" value="<?= htmlspecialchars($eventId) ?>">

                        <?php if (!$isLoggedIn): ?>
                            <input type="email" name="email" required placeholder="Enter your email"
                                class="px-4 py-3 rounded-full bg-gray-900 border border-white/20 text-white focus:outline-none focus:border-[<?= $colorValue ?>] min-w-[250px] text-sm">
                        <?php endif; ?>

                        <button type="submit" id="participateBtn"
                            class="px-6 py-3 rounded-full text-sm font-medium text-white transition-all duration-300 hover:scale-105 whitespace-nowrap w-full sm:w-auto"
                            style="background-color: <?= $colorValue ?>; box-shadow: 0 0 20px <?= $colorValue ?>40;">
                            <?= $isLoggedIn ? 'Confirm Participation' : 'Participate' ?>
                        </button>
                    </form>
                    <div id="participateMessage" class="hidden mt-3 text-sm text-center"></div>
                </div>
            </div>
        </footer>
        ...
        <script>
            document.getElementById('participateForm').addEventListener('submit', async function (e) {
                ...
                const msgDiv = document.getElementById('participateMessage');
                const formData = new FormData(this);

                btn.disabled = true;
                btn.innerHTML = 'Processing...';
                msgDiv.className = 'hidden mt-3 text-sm text-center';

                try {
                    const response = await fetch('../../api/participate-event.php', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    if (result.success) {
                        msgDiv.innerHTML = '<span class="text-green-400">' + result.message + '</span>';
                        ...
                        btn.disabled = false;
                        btn.innerHTML = '<?= $isLoggedIn ? 'Confirm Participation' : 'Participate' ?>';
                    }
                } catch (error) {
                    msgDiv.classList.remove('hidden');
                    msgDiv.innerHTML = '<span class="text-red-400">An error occurred. Please try again.</span>';
                    btn.disabled = false;
                    btn.innerHTML = '<?= $isLoggedIn ? 'Confirm Participation' : 'Participate' ?>';
                }
            });

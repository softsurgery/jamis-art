...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $resource = new Resource(null, $uploadId, $_POST['artTypeId'], $_POST['label'], $_POST['description']);
        $resourceController->save($resource);
        
        $success = true;
        $artTypeIdParam = $_POST['artTypeId']; // For redirect
    } catch (Exception $e) {
        $error = 'Error creating resource: ' . $e->getMessage();
    }
}
...
<body class="flex flex-col flex-1 h-screen w-screen overflow-hidden font-poppins">
    <?php if ($success): ?>
        <script>
            setTimeout(() => {
                window.location.href = '../resources.php?type=<?= urlencode($artTypeIdParam) ?>';
            }, 1000);
        </script>
    <?php endif; ?>
                ...
                <div class="container mx-auto rounded-lg shadow-sm p-8 max-w-3xl">
                    <div class="flex items-center justify-between pb-6 border-b border-gray-200 mb-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">Upload a new file and link it to an art type.</p>
                        </div>
                        <a href="../resources.php<?= $artTypeIdParam ? '?type=' . urlencode($artTypeIdParam) : '' ?>"
                    ...
                    <?php if ($success): ?>
                        <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-700">Resource created successfully. Redirecting...</p>
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!$error && !$success): ?>
                        <form method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="artTypeId" required
                                    ...
                                    <?php foreach ($artTypes as $type): ?>
                                        <option value="<?= $type['id'] ?>" <?= $artTypeIdParam == $type['id'] ? 'selected' : '' ?>>
                            ...
                            <div>
                                ...
                                <input type="text" name="label" required placeholder="e.g. Reference Image"
                                    class="w-full border border-gray-300 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-600/50 focus:border-indigo-600 transition bg-gray-50 focus:bg-white">
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500">Select the file you want to upload as a resource.</p>
                            </div>

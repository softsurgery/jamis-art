...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        $eventController->delete($_POST['id']);
...
<body class="flex flex-col flex-1 h-screen w-screen overflow-hidden">
            ...
            <div class="overflow-auto container mx-auto p-8 bg-gray-50 flex-1 relative">
                <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Manage all events and their details.</p>
                    </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($events as $event): ?>
                                ...
                                <tr class="hover:bg-gray-50 transition relative">
                                    <td class="px-6 py-4"><?= htmlspecialchars($event['id']) ?></td>
                                    <td class="px-6 py-4 font-medium text-gray-900">
                                        <?= htmlspecialchars($event['title']) ?>
                                    </td>
                                    <td class="px-6 py-4"><?= $artType ? htmlspecialchars($artType['label']) : 'N/A' ?></td>
                                    <td class="px-6 py-4 text-sm">
                                        <?= htmlspecialchars(date('M d, Y h:i A', strtotime($event['createdAt']))) ?>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-block relative">
                                            ...
                                            <div id="dropdown-<?= $event['id'] ?>"
                                                ...
                                                <button onclick="openDialog(<?= $event['id'] ?>)"
    ...
    <div id="delete-dialog"
        ...
        <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm scale-95 opacity-0 transition-all duration-200"
            ...
            <h2 class="text-xl font-bold text-gray-900 mb-2">Delete Event?</h2>
            <p class="text-gray-500 text-sm mb-6 leading-relaxed">Are you sure you want to delete this event? This

...
$controller = new SupportMessageController();
...
<body class="flex flex-col flex-1 h-screen w-screen overflow-hidden">
    <section class="flex flex-row flex-1 overflow-hidden">
        <div class="flex flex-col">
            <nav class="flex flex-col w-[15vw] h-full p-4">
                ...
                echo navLayout("support-messages", "../../");
                ?>
            </nav>
        </div>
        <main class="flex flex-col flex-1 overflow-hidden w-full">
            ...
            <div class="overflow-auto container mx-auto p-8 bg-gray-50 flex-1">
                <div class="flex items-center justify-between pb-4 border-b border-gray-200">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900">Support Messages</h1>
                        <p class="mt-1 text-sm text-gray-500">View and manage support inquiries.</p>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($messages as $msg): ?>
                                <tr class="hover:bg-gray-50 transition relative">
                                    <td class="px-6 py-4"><?= htmlspecialchars($msg->getId()) ?></td>
                                    <td class="px-6 py-4 font-medium text-gray-900 w-1/4">
                                        <?= htmlspecialchars($msg->getSubject()) ?>
                                    </td>
                                    <td class="px-6 py-4 w-1/3">
                                        <div class="line-clamp-2" title="<?= htmlspecialchars($msg->getMessage()) ?>">
                                            <?= htmlspecialchars($msg->getMessage()) ?>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <?= htmlspecialchars($msg->getEmail()) ?>
                                    </td>
                                    <td class="px-6 py-4">
                                        <?php
                                        if ($msg->getUserId()) {
                                            $user = $userController->getById($msg->getUserId());
                                            if ($user) {
                                                echo htmlspecialchars($user->getFirstName() . ' ' . $user->getLastName());
                                            } else {
                                                echo htmlspecialchars($msg->getUserId());
                                            }
                                        } else {
                                            echo '<span class="text-gray-400 italic">Guest</span>';
                                        }
                                        ?>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-block relative">
                                            <button onclick="toggleDropdown(<?= $msg->getId() ?>)"
                                                class="p-2 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition">
                                                <i data-lucide="more-vertical" class="w-4 h-4"></i>
                                            </button>
                                            <!-- dropdown -->
                                            <div id="dropdown-<?= $msg->getId() ?>"
                                                ...
                                                <button onclick="openDialog(<?= $msg->getId() ?>)"
    ...
    <div id="delete-dialog"
        ...
        <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm scale-95 opacity-0 transition-all duration-200"
            ...
            <h2 class="text-xl font-bold text-gray-900 mb-2">Delete Message?</h2>
            <p class="text-gray-500 text-sm mb-6 leading-relaxed">Are you sure you want to delete this message? This

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $user) {
    try {
        ...
        $updatedUser = new User($id, $firstName, $lastName, $email, '', $active, $role, $artTypeId);
        $userController->update($id, $updatedUser);

        if ($password !== '') {
            $passwordHash = password_hash($password, PASSWORD_BCRYPT);
            $userController->updatePassword($id, $passwordHash);
        }
                ...
                <div class="container mx-auto rounded-lg shadow-sm p-8">
                    <div class="flex items-center justify-between pb-6 border-b border-gray-200 mb-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">Update the user details below.</p>
                        </div>
                        <a href="../users.php"
                    ...
                    <?php elseif ($success): ?>
                        <div class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-green-700">User updated successfully. Redirecting...</p>
                            </div>
                        </div>
                    <?php else: ?>
                        <form method="POST" class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="password" name="password" placeholder="Leave blank to keep current password"
                                    class="w-full border border-gray-300 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-600/50 focus:border-indigo-600 transition">
                                <p class="mt-1 text-xs text-gray-500">Leave empty if you don't want to change the password.
                                </p>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="role" required
                                        class="w-full border border-gray-300 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-600/50 focus:border-indigo-600 transition">
                                        <option value="Standard" <?php echo ($user->getRole() ?? '') === 'Standard' ? 'selected' : ''; ?>>Standard</option>
                                ...
                                <div>
                                    ...
                                    <select name="active" required
                                        class="w-full border border-gray-300 rounded-lg p-3 text-sm outline-none focus:ring-2 focus:ring-indigo-600/50 focus:border-indigo-600 transition">
                                        <option value="1" <?php echo ($user->getActive() ?? 0) == 1 ? 'selected' : ''; ?>>
                                            Active
                                        </option>
                                        <option value="0" <?php echo ($user->getActive() ?? 0) == 0 ? 'selected' : ''; ?>>
                                            Inactive</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="artTypeId"
                                    ...
                                    <?php foreach ($artTypes as $type): ?>
                                        <option value="<?php echo $type['id']; ?>" <?php echo ($user->getArtTypeId() ?? null) == $type['id'] ? 'selected' : ''; ?>>
